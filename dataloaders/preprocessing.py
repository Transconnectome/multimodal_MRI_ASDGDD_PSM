...
def preprocessing_cat(subject_data, args):
    if args.cat_target:
        for cat in args.cat_target:
            if not str(subject_data[cat].unique()[0]).isnumeric():
                convert = dict()
                for name, num in zip(subject_data[cat].unique(), range(subject_data[cat].nunique())):
                    convert[name] = num
                subject_data[cat] = subject_data[cat].map(lambda x: convert[x])
            elif not 0 in list(subject_data.loc[:,cat]):
