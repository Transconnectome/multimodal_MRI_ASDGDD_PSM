...
class MultiModalImageDataset(Dataset, Randomizable):
    ...
    def __getitem__(self, index: int):
        self.randomize()
        meta_data, seg_meta_data, seg, label, confounder = None, None, None, None, None

        # load data and optionally meta
        img_keys = self.image_files.columns
        if self.image_only:
            imgs = tuple(map(self.loader, self.image_files.iloc[index])) # modified
        ...
        if self.transform is not None:
            ...
            if self.transform_with_metadata:
                img, meta_data = torch.stack((tuple(map(apply_transform, self.transform, imgs,
                                     repeat(False), repeat(True))))) # modified
            else:
                img = torch.stack((tuple(map(apply_transform, self.transform, imgs, repeat(False))))) # modified
        ...
        if self.confounder is not None:
            confounder = self.confounder.iloc[index]
