...
import torch
from monai.transforms import NormalizeIntensity, ScaleIntensity # Modified 
    ...
    def attribute(
        ...
        **kwargs: Any,
        ...
        def add_noise_to_input(input: Tensor, stdev: float, nt_samples_partition: int) -> Tensor:
            ...
            # TODO: Modified
            norm_input = NormalizeIntensity()(input.clone()) # to keep original input's mean
            noised_img = norm_input.repeat_interleave(nt_samples_partition, dim=0) + noise
            noised_img = ScaleIntensity()(noised_img)
            for i in range(bsz):
                noised_img[input.repeat_interleave(nt_samples_partition, dim=0) == 0] = 0
...
class CustomIG(IntegratedGradients):    
    def _attribute(
        self,
        inputs: Tuple[Tensor, ...],
        ...
        scaled_features_tpl = tuple(
            torch.cat(
                [baseline + float(alpha) * (input - baseline) for alpha in alphas], dim=0
            ).requires_grad_()
